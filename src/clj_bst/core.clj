...
(defn insert [{:keys [key value left right] :as tree} k v]
  (let [node (bst k v)]
    (cond
      (< key k) (assoc tree :right
                       (if right (insert right k v) node))
      (> key k) (assoc tree :left
                       (if left (insert left k v) node)))))
...
(defn delete [{:keys [left right key] :as tree} k]
  (cond
    ...
    :else (cond
            (and left right) (let [{sk :key sv :value sright :right} (smallest right)]
